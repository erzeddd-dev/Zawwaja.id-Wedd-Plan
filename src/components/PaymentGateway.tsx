...
export function PaymentGateway({ userId, userEmail, userFullName, onPaymentSuccess }: PaymentGatewayProps) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.transaction_status === 'settlement' || data.transaction_status === 'capture') {
            ...
            // Force close snap if it's still open
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const orderId = `ZAWWAJA-${userId}-${Date.now()}`;
      ...
      window.snap.pay(data.token, {
        onSuccess: function (result: any) {
          console.log("Payment success (from popup):", result);
          setPendingOrderId(null);
          onPaymentSuccess();
        },
        onPending: function (result: any) {
          console.log("Payment pending:", result);
          setError("Pembayaran sedang diproses (Pending). Silakan selesaikan pembayaran Anda. (Sistem akan otomatis mendeteksi jika lunas)");
        },
        ...
        onClose: function () {
          ...
          setIsLoading(false);
          // Don't clear pendingOrderId here, keep polling in background 
          // just in case they paid right as they closed it!
        }
      });
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-xs sm:max-w-md mx-auto" id="payment-gateway-wrapper">
      <div className="relative p-1.5 w-full bg-white/60 backdrop-blur-md rounded-[20px] shadow-xl border border-white/40 overflow-hidden">
        {/* Header / Info Area */}
